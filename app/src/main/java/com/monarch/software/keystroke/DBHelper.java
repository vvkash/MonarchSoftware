...
*/
        ...
        // Typing study sessions (Aalto-style transcription task)
        db.execSQL(TypingStudyDatabase.CREATE_SQL);

        // Swipe study sessions (FETA-style scroll gesture task)
        db.execSQL(SwipeStudyDatabase.CREATE_SQL);
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion < 2) {
            // Version 2: add typing study table (CREATE TABLE IF NOT EXISTS includes all columns)
            db.execSQL(TypingStudyDatabase.CREATE_SQL);
        }
        if (oldVersion < 3) {
            // Version 3: add IMU columns if they don't already exist (they exist if we came
            // through the v2 branch above since CREATE_SQL includes them)
            safeAddColumn(db, TypingStudyDatabase.TABLE_NAME, TypingStudyDatabase.COL_ACCEL_DATA, "TEXT");
            safeAddColumn(db, TypingStudyDatabase.TABLE_NAME, TypingStudyDatabase.COL_GYRO_DATA,  "TEXT");
            safeAddColumn(db, TypingStudyDatabase.TABLE_NAME, TypingStudyDatabase.COL_MAG_DATA,   "TEXT");
            db.execSQL(SwipeStudyDatabase.CREATE_SQL);
        }
        if (oldVersion < 4) {
            // Version 4: add participant_id to both study tables
            safeAddColumn(db, TypingStudyDatabase.TABLE_NAME, TypingStudyDatabase.COL_PARTICIPANT_ID, "TEXT");
            safeAddColumn(db, SwipeStudyDatabase.TABLE_NAME,  SwipeStudyDatabase.COL_PARTICIPANT_ID,  "TEXT");
        }
        if (oldVersion < 5) {
            // Version 5: add digraph and trigraph timing arrays to typing study
            safeAddColumn(db, TypingStudyDatabase.TABLE_NAME, TypingStudyDatabase.COL_DIGRAPH_DATA,  "TEXT");
            safeAddColumn(db, TypingStudyDatabase.TABLE_NAME, TypingStudyDatabase.COL_TRIGRAPH_DATA, "TEXT");
        }
    }

    /** ALTER TABLE ADD COLUMN, silently ignoring "duplicate column" errors. */
    private static void safeAddColumn(SQLiteDatabase db, String table, String column, String type) {
        try {
