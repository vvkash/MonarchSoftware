...
public class KeystrokeMainActivity extends AppCompatActivity {
    String app_password;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.ks_activity_pin);
...
//        new RetrieveFeedTask().execute(urlToRssFeed);
        ...
        mySwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    status = 1;
                    right_wrong.setTextColor(Color.parseColor("#BDB08A"));
                    right_wrong.setText("Re-train mode — enter PIN to add samples");
                } else {
                    status = 0;
                    // Trigger retrain with expanded data
                    DBHelper dbh = new DBHelper(KeystrokeMainActivity.this, Database.KEYSTROKE_DYNAMICS, null, Database.VERSION);
                    SQLiteDatabase dbs = dbh.getWritableDatabase();
                    Cursor cur = dbs.rawQuery("SELECT * from " + Database.TABLE_NAME, null);
                    ...
                    if (count > 0) {
                        ...
                        mProgressDialog.setMessage("Retraining neural network…");
                        mProgressDialog.show();
                        new Thread(() -> {
                            // Clear old impostor data and regenerate from expanded dataset
                            ...
                            runOnUiThread(() -> {
                                ...
                                right_wrong.setText("Retrained — enter PIN to test");
                                initialiseVariables();
                            });
                        }).start();
                    } else {
        ...
        ok.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (checksum && current_pointer >= app_password.length()) {
                    DBHelper mydbhelper = new DBHelper(KeystrokeMainActivity.this, Database.KEYSTROKE_DYNAMICS, null, Database.VERSION);
                    ...
                    if (r > 0) {
                        ...
                        if(count_training < 31 || stat == 0) {
                            if (count < 30) {
                                ...
                                right_wrong.setTextColor(Color.parseColor("#BDB08A"));
                                right_wrong.setText("Entry " + count + " / 30");
                                initialiseVariables();
                            } else {
                                ...
                                mProgressDialog.setCancelable(false);
                                mProgressDialog.setProgressStyle(ProgressDialog.STYLE_SPINNER);
                                mProgressDialog.setMessage("Training neural network…");
                                mProgressDialog.show();
                                ...
                                new Thread(() -> {
                                    ...
                                    runOnUiThread(() -> {
                                        mProgressDialog.dismiss();
                                        right_wrong.setTextColor(Color.parseColor("#4CAF50"));
                                        right_wrong.setText("Training complete — enter PIN to test");
                                        initialiseVariables();
                                    });
                                }).start();
                            }
                            ...
                            if (status == 1) {
                                // Re-train mode: record as extra training sample
                                Cursor reCur = mydatabase.rawQuery("SELECT * from " + Database.TABLE_NAME, null);
                                int reCount = reCur.getCount();
                                reCur.close();
                                right_wrong.setTextColor(Color.parseColor("#BDB08A"));
                                right_wrong.setText("Sample " + reCount + " added — toggle off to retrain");
                                initialiseVariables();
                            } else {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.menu_keystroke, menu);
        return true;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.keystroke) {
            Intent i = new Intent(KeystrokeMainActivity.this, ShowValuesActivity.class);
            startActivity(i);
            return true;
        } else if (id == R.id.weight) {
            Intent in = new Intent(KeystrokeMainActivity.this, ShowWeightsActivity.class);
            startActivity(in);
            return true;
        } else if (id == R.id.typing_study) {
            Intent ts = new Intent(KeystrokeMainActivity.this, TypingStudyActivity.class);
            startActivity(ts);
            return true;
        } else if (id == R.id.typing_results) {
            Intent tr = new Intent(KeystrokeMainActivity.this, TypingResultsActivity.class);
            startActivity(tr);
            return true;
        } else if (id == R.id.swipe_study) {
            Intent ss = new Intent(KeystrokeMainActivity.this, SwipeStudyActivity.class);
            startActivity(ss);
            return true;
        } else if (id == R.id.swipe_results) {
            Intent sr = new Intent(KeystrokeMainActivity.this, SwipeResultsActivity.class);
            startActivity(sr);
            return true;
        } else if (id == R.id.export_data) {
            Intent ex = new Intent(KeystrokeMainActivity.this, ExportActivity.class);
            startActivity(ex);
            return true;
            ...
            new android.app.AlertDialog.Builder(this)
                ...
                .setPositiveButton("Restart", (dialog, which) -> {
                    DBHelper dbHelper = new DBHelper(KeystrokeMainActivity.this, Database.KEYSTROKE_DYNAMICS, null, Database.VERSION);
                    SQLiteDatabase db = dbHelper.getWritableDatabase();
                    db.delete(Database.TABLE_NAME, null, null);
                    db.delete(Database.ILLEGITIMATE_TABLE_NAME, null, null);
                    db.delete(Database.TRAINING_TABLE_NAME, null, null);
                    db.delete(Database.PASSWORD_TABLE_NAME, null, null);
                    db.delete(DatabaseNetwork.NETWORK_TABLE_NAME, null, null);
                    right_wrong.setText("");
                    attempt.setText("#Entry No. 1");
                    initialiseVariables();
                    Toast.makeText(KeystrokeMainActivity.this, "Auth reset — set a new PIN", Toast.LENGTH_SHORT).show();
                })
                    ...
                    public void onClick(DialogInterface arg0, int arg1) {
                        ...
                        if(password.getText().toString().length()==6) {
                            ...
                            if (password.getText().toString().equals(retype_password.getText().toString())) {
                                ...
                                mydbhelper.close();
                                Toast.makeText(KeystrokeMainActivity.this, "Code set successfully", Toast.LENGTH_SHORT).show();

                            } else {
                                Toast.makeText(KeystrokeMainActivity.this, "Codes do not match. Please try again", Toast.LENGTH_SHORT).show();
                                setPassword();

                            }
                        }else{
                            Toast.makeText(KeystrokeMainActivity.this, "Code Length should be 6 Characters only.", Toast.LENGTH_SHORT).show();
                            setPassword();
                            }
                    }
                });
